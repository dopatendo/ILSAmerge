...
First, as an example, using `ILSAdownload()` we can download the datasets of 
'RLII' 1991:
```{r}
ILSAdownload(study = "RLII", year = 1991, outputdir = tempdir(),
             unzip = TRUE, agreeLicense = TRUE)
```

In the case of 'RLII' 1991, we have X files and only 1 population:
```{r}
ILSAfile.info(inputdir = file.path(tempdir(),"RLII1991_IDB_SPSS/Data"))
```

With `ILSAmerge()` we can combine all the files into the number of populations
we have.

For example, let us create a new path, where the merged files will be 
saved:
